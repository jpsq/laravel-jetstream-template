<script setup>
    ...
    const uploadFile = (file) => {
        ...
        return new Promise((resolve, reject) => {
            axios
                .post(route('media.upload'), formData)
...
<template>
    <div
        ...
        @drop.prevent="onDrop"
        @dragover.prevent
        :class="{ 'border-border bg-surface': !dragEnter, 'border-cyan-500 bg-cyan-50': dragEnter }"
    ...
    >
        <div class="relative flex flex-col justify-center">
            <div v-if="dragEnter" @dragleave.prevent="dragEnter = false" @dragover.prevent class="w-full h-full absolute"></div>
            <Icon
                name="tabler-upload"
                :class="{ 'text-muted-foreground': !dragEnter, 'text-cyan-500': dragEnter }"
            ...
            <div class="text-sm text-muted-foreground text-center">{{ mimeTypes.join(', ') }}</div>
        </div>
        <Preloader v-if="isLoading" size="xl" class="rounded-lg" />
    </div>

    <input ref="input" id="browse" type="file" :accept="mimeTypes.join(',')" multiple="multiple" class="hidden" @change="onBrowse" />

    <div v-if="completedJobs.length" class="mt-6">
