...
const typesTarget = join(__dirname, 'types.d.ts');
...
(async function () {
    ...
    try {
        ...
        await fs.mkdir(dirname(typesTarget), { recursive: true });
    } catch (err) {
        //
    }
        ...
        for (let i = 0; i < sources.json.length; i++) {
            ...
            for (const key in content) {
                if (key === 'prefix' && content.prefix === 'tabler') {
                    for (const k in content.icons) content.icons[k].body = content.icons[k].body.replace(/stroke-width="2"/g, 'stroke-width="1.5"');
                }
            }
    ...
    // Generate types
    const iconsNames = allIcons.map((iconSet) => Object.keys(iconSet.icons).map((name) => `${iconSet.prefix}-${name}`)).flat();

    const typesContent = `export type IconifyIconName = ${iconsNames.map((name) => `"${name}"`).join(' | ')};`;

    // Save the types to a file
    await fs.writeFile(typesTarget, typesContent, 'utf8');
    console.log(`Saved types to ${typesTarget}!`);
})().catch((err) => {
    console.error(err);
});
